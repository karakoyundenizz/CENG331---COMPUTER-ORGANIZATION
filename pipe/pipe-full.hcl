...
## You can implement the testq instruction if you like. Pretty easy since
## most of the issues are already handled for you. Could be useful in your
## to_morse_digit_string.ys code!
...
# Instruction code for testq instruction
...
#
##################### MODIFICATIONS ###########################
# 1. I implemented 'cmpq' function like in x86-64 by redefining the 'testq' (ITESTQ) instruction.
#    - Reason for modification: The baseline Y86 set lacks a compare instruction that does not
#      overwrite a destination register. Standard subtraction (subq) forces a 
#      write-back, requiring register saving/restoring which increases instruction count.
#	   so I needed to first assign a value to limit register,then save my control register to another register
#	   in order to not loose the original content when I use OPERATIONAL instructions. So by developing this strategy
# 	   I may be improved my design in hardware so that my CPE goes lower.
#
#    - Implementation Details:
#      a. Fetch Stage: Added ITESTQ to 'instr_valid' and 'need_regids' to make sure
#         the instruction is recognized and rA/rB are fetched.
#      b. Decode Stage: Re-arranged 'd_srcA' and 'd_srcB' so that we read both operand values.
#         Crucially, 'd_dstE' is NOT set for ITESTQ, effectively disabling write-back (if it is set there will be no difference between basic operational instructions ).
#      c. Execute Stage: Changed 'alufun' for ITESTQ from ALUAND (default testq)
#         to ALUSUB (subtraction). This calculates (valB - valA, order is important cmpq valA, valB) to set condition codes
#         without storing the result and changing any of the registers.
#      d. Condition Codes: Added ITESTQ to 'set_cc' to ensure flags (ZF, SF, OF)
#         are updated based on the subtraction result.
#
#    - Achieved Feautre: This allows the assembly code to perform "testq %rA, %rB" to check
#      conditions like (rB < rA) without needing extra 'rrmovq' instructions to
#      preserve values, significantly reducing the Cycle Per Element(CPE).
...
word d_srcA = [
D_icode in { IRRMOVQ, IRMMOVQ, IOPQ, IPUSHQ, ITESTQ } : D_rA;
...
word d_srcB = [
D_icode in { IOPQ, IRMMOVQ, IMRMOVQ, ITESTQ } : D_rB;
...
word aluA = [
    E_icode in { IRRMOVQ, IOPQ, ITESTQ } : E_valA;
...
E_icode in { IRMMOVQ, IMRMOVQ, IOPQ, ICALL, 
		     IPUSHQ, IRET, IPOPQ, ITESTQ } : E_valB;
...
word alufun = [
	E_icode == IOPQ : E_ifun;
	E_icode == ITESTQ :ALUSUB;
	1 : ALUADD;
];

## Should the condition codes be updated?
bool set_cc = (E_icode == IOPQ  || E_icode == ITESTQ) &&
