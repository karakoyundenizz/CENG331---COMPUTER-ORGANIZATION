...
#
# correctness.pl - Test to_morse_digit_string assembly code for correctness
#
use Getopt::Std;
...
if (!$opt_f) {
    $tobinary = "to_morse_digit_string";
} else {
    $tobinary = $opt_f;
    # Strip off .ys
    $tobinary =~ s/\.ys//;
...
for ($i = 0; $i <= $blocklen+$over; $i++) {
    ...
    if ($stat =~ "dddd") {
	$result = "Incorrect Morse encoding";
    }
    if ($stat =~ "eeee") {
