...
instr_t instruction_set[] = 
{
	...
	{"testq",  HPACK(I_TESTQ, F_NONE), 2, R_ARG, 1, 1, R_ARG, 1, 0},
...
stat_t step_state(state_ptr s, FILE *error_file)
{
    ...
    if (!reg_valid(hi1) || !reg_valid(lo1)) {
        if (error_file)
            fprintf(error_file,"PC = 0x%llx, Invalid register ID\n", s->pc);
        return STAT_INS;
    }
    argA = get_reg_val(s->r, hi1);
    argB = get_reg_val(s->r, lo1);
    s->cc = compute_cc(A_AND, argA, argB);
    s->pc = ftpc;
    break;
