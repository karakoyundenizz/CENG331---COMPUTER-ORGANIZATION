...
char fully_unrolled_descr[] = "bilateral: unrolled every fucking loop maxing the divided version  ";

void fully_unrolled_bilateral(int dim, pixel *src, pixel *dst)
{
    int BLOCK = 32;
    ...
    for (i_blocked = radius; i_blocked < block_limit; i_blocked += BLOCK)
    {
        for (j_blocked = radius; j_blocked < block_limit; j_blocked += BLOCK)
        {
            ...
            for (i_in_block = i_blocked; i_in_block < i_end; i_in_block++)
            {
                ...
                pixel *row0 = src + (i_in_block - 2) * dim;
                pixel *row1 = src + (i_in_block - 1) * dim;
                pixel *row2 = src + i_in_block * dim; // middle pointer so we can clculate just by using it
                pixel *row3 = src + (i_in_block + 1) * dim;
                pixel *row4 = src + (i_in_block + 2) * dim;

                for (j_in_block = j_blocked; j_in_block < j_end; j_in_block++)
                {
                    // PICK THE MIDDLE ROW TO CALCULATE OTHERS
                    pixel current_pixel = row2[j_in_block];
                    ...
                    /* fully unrolled 5x5 */
                    ACC(row0[j_in_block - 2], 0, 0);
                    ACC(row0[j_in_block - 1], 0, 1);
                    ACC(row0[j_in_block], 0, 2);
                    ACC(row0[j_in_block + 1], 0, 3);
                    ACC(row0[j_in_block + 2], 0, 4);
                    ACC(row1[j_in_block - 2], 1, 0);
                    ACC(row1[j_in_block - 1], 1, 1);
                    ACC(row1[j_in_block], 1, 2);
                    ACC(row1[j_in_block + 1], 1, 3);
                    ACC(row1[j_in_block + 2], 1, 4);
                    ACC(row2[j_in_block - 2], 2, 0);
                    ACC(row2[j_in_block - 1], 2, 1);
                    ACC(row2[j_in_block], 2, 2);
                    ACC(row2[j_in_block + 1], 2, 3);
                    ACC(row2[j_in_block + 2], 2, 4);
                    ACC(row3[j_in_block - 2], 3, 0);
                    ACC(row3[j_in_block - 1], 3, 1);
                    ACC(row3[j_in_block], 3, 2);
                    ACC(row3[j_in_block + 1], 3, 3);
                    ACC(row3[j_in_block + 2], 3, 4);
                    ACC(row4[j_in_block - 2], 4, 0);
                    ACC(row4[j_in_block - 1], 4, 1);
                    ACC(row4[j_in_block], 4, 2);
                    ACC(row4[j_in_block + 1], 4, 3);
                    ACC(row4[j_in_block + 2], 4, 4);
